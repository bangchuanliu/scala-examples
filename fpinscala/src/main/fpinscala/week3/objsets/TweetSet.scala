...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    ...
    val inlCur = if (p(elem)) leftAcc.incl(elem) else leftAcc
    right.filterAcc(p, inlCur)
  }
  ...
  def union(that: TweetSet): TweetSet = {
    ...
    val inlCur = if (leftUnion.contains(elem)) leftUnion else leftUnion.incl(elem)
    right.union(inlCur)
//    left.union(right.union(that)).incl(elem)
  ...
  override def mostRetweeted: Tweet = {
    ...
    if (elem.retweets > leftTweet.retweets && elem.retweets > rightTweet.retweets) {
      elem
  ...
  override def descendingByRetweet: TweetList = {
    val t = mostRetweeted
    val newSet = remove(t)
    val list = newSet.descendingByRetweet
    new Cons(t, list)
  }
}
